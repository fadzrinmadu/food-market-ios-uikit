//
//  ImageConstant.swift
//  food-market-ios
//
//  Created by Mac 2020 on 30/05/25.
//

import UIKit

enum ImageConstant {
    static let home: UIImage = UIImage(named: "Home") ?? UIImage()
    static let homeFilled: UIImage = UIImage(named: "HomeFilled") ?? UIImage()
    static let profile: UIImage = UIImage(named: "Profile") ?? UIImage()
    static let profileFilled: UIImage = UIImage(named: "ProfileFilled") ?? UIImage()
    static let order: UIImage = UIImage(named: "Order") ?? UIImage()
    static let orderFilled: UIImage = UIImage(named: "OrderFilled") ?? UIImage()
}
