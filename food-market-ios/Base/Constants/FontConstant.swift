//
//  FontConstant.swift
//  food-market-ios
//
//  Created by Mac 2020 on 29/09/24.
//

import UIKit

enum FontConstant {
    static let poppinsMediumSmall = UIFont(name: "Poppins-Medium", size: 14)
    static let poppinsRegularLarge = UIFont(name: "Poppins-Regular", size: 20)
    static let poppinsRegularMedium = UIFont(name: "Poppins-Regular", size: 18)
    static let poppinsRegular = UIFont(name: "Poppins-Regular", size: 16)
    static let poppinsRegularSmall = UIFont(name: "Poppins-Regular", size: 14)
    static let poppinsRegularXSmall = UIFont(name: "Poppins-Regular", size: 12)
    static let poppinsLightSmall = UIFont(name: "Poppins-Light", size: 14)
}
